...
type Decoder struct {
	...
	Vertices        []mgl32.Vec3         // vertices positions array
	Uvs             []mgl32.Vec2         // vertices texture coordinates
	...
	rexBlockCounter uint64               // The internal counter used for writing mesh blocks
}
...
type Material struct {
	...
	Ambient    Color   // Ambient color reflectivity
	Diffuse    Color   // Diffuse color reflectivity
	Specular   Color   // Specular color reflectivity
...
var defaultMat = &Material{
	Diffuse:   Color{R: 0.7, G: 0.7, B: 0.7},
	Ambient:   Color{R: 0.7, G: 0.7, B: 0.7},
...
// WriteREX writes the content to a REX writer
func (dec *Decoder) WriteREX(w io.Writer) error {

	var file rexfile.File

	// Write default material block
	dec.rexBlockCounter = defaultMaterialID
	file.Materials = append(file.Materials, rexfile.NewMaterial(dec.rexBlockCounter))
	dec.rexBlockCounter++

	for _, v := range dec.Objects {

		err := dec.writeObjectToREX(&v, &file)
		if err != nil {
			return err
		}
	}

	encoder := rexfile.NewEncoder(w)
	return encoder.Encode(file)
}

func (dec *Decoder) createMeshGeometry(obj *Object) (rexfile.Mesh, error) {

	var mesh rexfile.Mesh
	vertexMap := make(map[int]int)

	mesh.Name = obj.Name
	mesh.ID = dec.rexBlockCounter
	dec.rexBlockCounter++
	// assign default material
	mesh.MaterialID = defaultMaterialID

	fmt.Println("Object:", obj.Name, " ", len(obj.materials))

	for i := 0; i < len(dec.Vertices); i++ {
		fmt.Println(dec.Vertices[i])
	...
	if len(obj.materials) == 1 {
		...
		for _, face := range obj.Faces {
			...
			// TODO check vertices (if >3 just triangulate)

			var newIdx [3]int
			var ok bool
			for i := 0; i < 3; i++ {
				newIdx[i], ok = vertexMap[face.Vertices[i]]
				if !ok {
					mesh.Coords = append(mesh.Coords, mgl32.Vec3{
						// X: obj.Vertices[]
					})
					vertexMap[face.Vertices[i]] = len(mesh.Coords) - 1
					newIdx[i] = vertexMap[face.Vertices[i]]
				}
...
func (dec *Decoder) writeObjectToREX(obj *Object, rexFile *rexfile.File) error {

	// Creates mesh geometry block
	mesh, err := dec.createMeshGeometry(obj)
	if err != nil {
		return err
	}
...
func (dec *Decoder) parseVertex(fields []string) error {
	...
	dec.Vertices = append(dec.Vertices, mgl32.Vec3{v[0], v[1], v[2]})
	return nil
}
...
func (dec *Decoder) parseNormal(fields []string) error {
	...
	// Normals are ignored, since they are calculated in real-time on the client
	return nil
}
...
func (dec *Decoder) parseTex(fields []string) error {
	...
	dec.Uvs = append(dec.Uvs, mgl32.Vec2{v[0], v[1]})
	return nil
}
